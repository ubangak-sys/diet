...
3. Возможности:
   ...
   - **Семейный ИИ-совет** — рекомендации для всей семьи сразу с учётом
     предпочтений и аллергий каждого члена.
...
│   ├── functions/ai-advice/index.ts  # Edge Function ИИ (личный+семейный)
│   └── config.toml
├── src/
│   ├── app/
...
│   │       └── advice/page.tsx       # ИИ-советы (личный + семейный)
...
- `family_members` — участники семей (одна семья на пользователя).
- `family_advice` — семейный ИИ-совет (уникальный на семью+дату).
