...
каждый день подсказывает, какие новые блюда и продукты стоит добавить.
...
3. **Отключите подтверждение email** (для быстрого старта; в продакшене —
   оставьте включённым): **Authentication → Providers → Email** → выключите
   «Confirm email».
4. Возьмите ключи проекта: **Project Settings → API**:
...
│   ├── functions/ai-advice/index.ts  # Edge Function для ИИ
│   └── config.toml
├── src/
│   ├── app/
...
│   │       ├── page.tsx              # дашборд
│   │       ├── log/page.tsx          # дневник питания
│   │       ├── preferences/page.tsx  # предпочтения
│   │       └── advice/page.tsx       # ИИ-советы
...
- `preferences` — предпочтения (одна строка на пользователя).
- `meal_entries` — ежедневный лог приёмов пищи.
- `daily_advice` — ежедневный ИИ-совет (уникальный на пользователя+дату).

Безопасность: включён Row Level Security — каждый пользователь видит и меняет
только свои данные.
